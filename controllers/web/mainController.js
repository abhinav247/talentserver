    ...
    eventList.forEach(async (event, index) => {
      if (index === eventIndex) {
         ...
         Event.findOneAndUpdate(
          ...
          console.log(moment(raw.event_endDate).format('MMMM Do YYYY, h:mm:ss a'));
        });
      } 
...
router.put("/job/:id", (req, res, nex) => {
  ...
  Job.findOneAndUpdate(
    ...
    .then(async raw => {
      let updatedJob = await Job.findById(req.params.id).populate("eventList");
      res.send({
...
router.post("/event", async (req, res, next) => {
  ...
  let event = await Event.create(eventObj).catch(error => console.log(error));
